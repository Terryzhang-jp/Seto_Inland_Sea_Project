...
主要船运公司：四国汽船、ジャンボフェリー、国際両備フェリー、四国フェリー、雌雄島海運、豊島フェリー、小豆島豊島フェリー
    ...
    def load_ferry_routes(self):
        ...
        if self.routes_data is None:
            self.load_data()
        return self.routes_data.to_dict('records')

    def load_ports(self):
        """加载港口数据"""
        if self.ports_data is None:
            self.load_data()
        return self.ports_data.to_dict('records')

    def load_companies(self):
        """加载公司数据"""
        if self.companies_data is None:
            self.load_data()
        return self.companies_data.to_dict('records')

    def load_popular_routes(self):
        """加载热门路线数据"""
        if self.popular_routes_data is None:
            self.load_data()
        return self.popular_routes_data
